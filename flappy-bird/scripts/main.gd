...
func _ready() -> void:
	...
	_setup_ground()
	_spawn_timer.wait_time = GAME_CONFIG.PIPE_SPAWN_INTERVAL
...
func _on_spawn_timer_timeout() -> void:
	...
	pipe_pair.position.x = GAME_CONFIG.PIPE_SPAWN_X
	pipe_pair.gap_center_y = randf_range(GAME_CONFIG.GAP_Y_MIN, GAME_CONFIG.GAP_Y_MAX)
	_pipe_container.add_child(pipe_pair)


func _setup_ground() -> void:
	var ground_shape := RectangleShape2D.new()
	ground_shape.size = Vector2(GAME_CONFIG.SCREEN_WIDTH, GAME_CONFIG.GROUND_HEIGHT)
	_ground_collision.shape = ground_shape
	_ground.position = Vector2(
		GAME_CONFIG.SCREEN_WIDTH / 2.0,
		GAME_CONFIG.GROUND_TOP_Y + GAME_CONFIG.GROUND_HEIGHT / 2.0
	)


func _draw() -> void:
	# Ground base
	var ground_y: float = GAME_CONFIG.GROUND_TOP_Y
	draw_rect(Rect2(0, ground_y, GAME_CONFIG.SCREEN_WIDTH, GAME_CONFIG.GROUND_HEIGHT), Color(0.545, 0.271, 0.075))

	# Grass stripe on top of ground
	draw_rect(Rect2(0, ground_y, GAME_CONFIG.SCREEN_WIDTH, GAME_CONFIG.GROUND_GRASS_HEIGHT), Color(0.133, 0.545, 0.133))

	# Ground texture lines
	for i in range(0, int(GAME_CONFIG.SCREEN_WIDTH), 20):
		draw_line(
