...
@export var projectile_weapon_id: StringName = &"boss_projectile"
@export var projectile_attack_id: StringName = &"projectile"
@export var projectile_color := Color(0.67, 0.92, 1.0, 1.0)
@export var projectile_visual_scale := Vector2.ONE
@export var contact_damage := 4
@export var contact_knockback := Vector2(180.0, -110.0)
@export var contact_weapon_id: StringName = &"boss_body"
@export var phase_two_health_ratio := 0.5
@export var phase_two_tint := Color(0.95, 0.86, 1.0, 1.0)
@export var defeated_tint := Color(0.62, 0.62, 0.7, 1.0)
...
func _refresh_visuals() -> void:
	...
	match _phase:
		PHASE_TWO:
			visual_root.modulate = phase_two_tint
		PHASE_DEFEATED:
			visual_root.modulate = defeated_tint
		_:
...
func _fire_projectile() -> void:
	...
	if projectile.has_method("configure"):
		projectile.configure({
			...
			"visual_scale": projectile_visual_scale,
			"knockback": projectile_knockback,
		})
	attack_fired.emit(_phase, projectile_attack_id)
...
func _on_health_changed(current_health: int, max_health: int) -> void:
	...
	if not _is_active or _phase != PHASE_ONE:
		return
	if current_health <= maxi(1, int(ceil(float(max_health) * phase_two_health_ratio))):
