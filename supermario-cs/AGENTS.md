...
Pickups emit events, never mutate state. For example, `Coin` calls `GameSession.Current.Events.EmitScoreEarnedAt(points, GlobalPosition)`; `GameSession` applies the score and spawns the `ScorePopup`.
