		...
		for child in node.get_children():
			if child is HBoxContainer and not has_expand_btn:
				for btn in child.get_children():
					if btn is Button and btn.text == "Expand All":
						has_expand_btn = true
						hbox = child
						break
...
func _inject_copy_button() -> void:
	...
	for i in _hbox.get_child_count():
		...
		if child is Button and child.text == "Collapse All":
			insert_idx = i + 1
			break
...
func _get_type_char(item: TreeItem) -> String:
	# Check TreeItem metadata first (Godot stores "warning" / "error" / "cycled_error")
	...
	# Fall back to inferring from child text
	var child := item.get_first_child()
	while child:
